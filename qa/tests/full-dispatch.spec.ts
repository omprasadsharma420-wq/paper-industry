...
async function openOrder(page: Page, orderNo: string) {
  ...
  await page.getByPlaceholder("Search orders").fill(orderNo);
...
test.describe.serial("200-diary production demonstration", () => {
  ...
  test("completes the role-to-role dispatch and leaves exactly 50 available", async ({ browser }) => {
    ...
    await orderDialog.getByLabel(/Dispatch date/).fill(futureDate());
    await orderDialog.getByLabel(/Priority/).selectOption("NORMAL");
    await orderDialog.getByLabel(/Source/).selectOption("FINISHED_STOCK");
    ...
    evidence.push(await captureAction(sales.page, () => sales.page.getByRole("button", { name: /Send for stock check/ }).click()));
    ...
    evidence.push(await captureAction(quality.page, () => quality.page.getByRole("button", { name: "Check stock" }).click()));

    const supervisor = await rolePage(browser, "Supervisor");
    sessions.push(supervisor);
    await openOrder(supervisor.page, orderNo);
    evidence.push(await captureAction(supervisor.page, () => supervisor.page.getByRole("button", { name: /Approve & reserve/ }).click()));
    ...
    evidence.push(await captureAction(packing.page, () => packing.page.getByRole("button", { name: "Finish picking" }).click()));

    await quality.page.reload({ waitUntil: "domcontentloaded" });
    await expect(quality.page.getByRole("heading", { name: orderNo })).toBeVisible();
    await quality.page.getByRole("button", { name: "Record quality check" }).click();
    const qcDialog = quality.page.getByRole("dialog", { name: "Quality check" });
    await qcDialog.getByLabel(/Result/).selectOption("PASSED");
    for (const label of ["Correct page count", "Dimensions correct", "Binding secure", "Cover correct", "Branding correct", "Pages clean", "No torn pages"]) {
      await qcDialog.getByRole("checkbox", { name: label }).check();
    }
    evidence.push(await captureAction(quality.page, () => qcDialog.getByRole("button", { name: "Save" }).click()));
    ...
    await packing.page.getByRole("button", { name: "Finish packing" }).click();
    ...
    evidence.push(await captureAction(packing.page, () => packingDialog.getByRole("button", { name: "Save" }).click()));
    ...
    await expect(packing.page.getByRole("button", { name: "Check documents" })).toBeVisible({ timeout: 8_000 });
    await packing.page.getByRole("button", { name: "Check documents" }).click();
