...
declare
  ...
  v_allowed boolean := public.demo_action_allowed(p_actor_role, p_action);
begin
  if v_allowed and p_action = 'VERIFY_WEIGHT' and p_action_input ? 'actualWeightKg' then
    update dispatch_requests
    set actual_weight_kg = nullif(p_action_input->>'actualWeightKg', '')::numeric
    where id = p_dispatch_id;
  end if;

  if v_allowed and p_action = 'VERIFY_DOCUMENTS' and jsonb_typeof(p_action_input->'documents') = 'array' then
    for v_document in
  ...
  if v_allowed and p_action = 'ASSIGN_VEHICLE' and v_ok then
  ...
  if v_allowed and v_note is not null and p_action in ('REJECT', 'RESOLVE_EXCEPTION', 'CANCEL') then
    update audit_logs
    set note = v_note
    where id = (
      select id
